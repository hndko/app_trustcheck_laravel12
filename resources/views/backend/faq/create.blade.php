...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="flex items-center gap-4 bg-[#1E293B] p-6 rounded-2xl border border-[#334155] shadow-sm">
        ...
        <div>
            <h2 class="text-lg font-bold text-white">Tambah FAQ Baru</h2>
            <p class="text-xs text-[#94A3B8] mt-0.5">Lengkapi pertanyaan dan jawaban penjelasan untuk pusat bantuan publik.</p>
        </div>
    </div>
    ...
    <div class="bg-[#1E293B] rounded-2xl border border-[#334155] p-6 shadow-sm">
        <form action="{{ route('portal.faq.store') }}" method="POST" class="space-y-6">
            @csrf
            <div>
                ...
                <input type="text" name="question" id="question" value="{{ old('question') }}" required
                    placeholder="Contoh: Apa itu Trust Score AI?"
            ...
            <div>
                ...
                <textarea name="answer" id="answer" rows="5" required
                    placeholder="Tuliskan jawaban penjelasan yang jelas dan baku..."
                    class="w-full px-4 py-3 rounded-xl bg-[#0F172A] border border-[#334155] text-white placeholder-[#64748B] text-sm focus:outline-none focus:border-[#38BDF8] transition-colors">{{ old('answer') }}</textarea>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="flex items-center pt-6">
                    <label class="flex items-center gap-3 cursor-pointer text-sm font-semibold text-[#E2E8F0]">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded bg-[#0F172A] border-[#334155] text-[#2563EB] focus:ring-0 w-5 h-5">
            ...
            <div class="pt-4 border-t border-[#334155] flex justify-end gap-3">
                ...
                <button type="submit" class="px-6 py-2.5 rounded-xl bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-xs font-extrabold shadow-sm transition-all cursor-pointer">Simpan FAQ</button>
            </div>
        </form>
    </div>
</div>
